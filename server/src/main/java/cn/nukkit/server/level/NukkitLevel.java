...
import cn.nukkit.server.level.generator.task.*;
import cn.nukkit.server.level.particle.DestroyBlockParticle;
import cn.nukkit.server.level.particle.Particle;
import cn.nukkit.server.level.sound.BlockPlaceSound;
import cn.nukkit.server.level.sound.Sound;
import cn.nukkit.server.math.*;
import cn.nukkit.server.math.BlockFace.Plane;
import cn.nukkit.server.metadata.BlockMetadataStore;
 ...
 */
public class NukkitLevel implements ChunkManager, Metadatable, cn.nukkit.api.level.Level {
    ...
    public NukkitLevel(NukkitServer server, String name, String path, Class<? extends LevelProvider> provider) {
        ...
        try {
            if (convert) {
                ...
                this.provider = provider.getConstructor(NukkitLevel.class, String.class).newInstance(this, newPath);
            } else {
                this.provider = provider.getConstructor(NukkitLevel.class, String.class).newInstance(this, path);
            }
        } catch (Exception e) {
    ...
    public void addSound(Sound sound, Player[] players) {
        ...
        if (players == null) {
            ...
            if (packets != null) {
                if (packets.length == 1) {
                    NukkitServer.broadcastPacket(players, packets[0]);
                } else {
    ...
    public boolean unload(boolean force) {
        ...
        NukkitLevel defaultLevel = this.server.getDefaultLevel();
        ...
        for (long index : this.chunkPackets.keySet()) {
            ...
            if (chunkPlayers.length > 0) {
                for (DataPacket pk : this.chunkPackets.get(index)) {
                    NukkitServer.broadcastPacket(chunkPlayers, pk);
                }
            }
        }
    ...
    public void checkSleep() {
        ...
        if (resetTime) {
            int time = this.getTime() % NukkitLevel.TIME_FULL;

            if (time >= NukkitLevel.TIME_NIGHT && time < NukkitLevel.TIME_SUNRISE) {
                this.setTime(this.getTime() + NukkitLevel.TIME_FULL - time);
    ...
    public void sendBlocks(Player[] target, Vector3[] blocks, int flags, boolean optimizeRebuilds) {
        ...
        if (optimizeRebuilds) {
            ...
            for (Vector3 b : blocks) {
                ...
                long index = NukkitLevel.chunkHash((int) b.x >> 4, (int) b.z >> 4);
    ...
    public void clearChunkCache(int chunkX, int chunkZ) {
        this.chunkCache.remove(NukkitLevel.chunkHash(chunkX, chunkZ));
    }
    ...
    private void tickChunks() {
        ...
        for (ChunkLoader loader : this.loaders.values()) {
            ...
            for (int chunk = 0; chunk < chunksPerLoader; ++chunk) {
                ...
                long hash = NukkitLevel.chunkHash(dx + chunkX, dz + chunkZ);
    ...
    public Block getBlock(Vector3 pos, boolean cached) {
        long chunkIndex = NukkitLevel.chunkHash((int) pos.x >> 4, (int) pos.z >> 4);
        BlockVector3 index = NukkitLevel.blockHash((int) pos.x, (int) pos.y, (int) pos.z);
        int fullState;
    ...
    public void updateBlockLight(int x, int y, int z) {
        ...
        if (oldLevel != newLevel) {
            ...
            if (newLevel < oldLevel) {
                removalVisited.put(NukkitLevel.blockHash(x, y, z), true);
                lightRemovalQueue.add(new Object[]{new Vector3(x, y, z), oldLevel});
            } else {
                visited.put(NukkitLevel.blockHash(x, y, z), true);
    ...
    private void computeRemoveBlockLight(int x, int y, int z, int currentLight, Queue<Object[]> queue,
        ...
        BlockVector3 index = NukkitLevel.blockHash(x, y, z);
    ...
    public boolean setBlock(Vector3 pos, Block block, boolean direct, boolean update) {
        ...
        if (this.getChunk((int) pos.x >> 4, (int) pos.z >> 4, true).setBlock((int) pos.x & 0x0f, (int) pos.y & 0xff,
            ...
            this.blockCache.remove(NukkitLevel.blockHash((int) position.x, (int) position.y, (int) position.z));

            Long index = NukkitLevel.chunkHash((int) position.x >> 4, (int) position.z >> 4);
    ...
    private void addBlockChange(int x, int y, int z) {
        long index = NukkitLevel.chunkHash(x >> 4, z >> 4);
    ...
    private void addBlockChange(long index, int x, int y, int z) {
        ...
        if (currentMap != changeBlocksFullMap && currentMap != null) {
            if (currentMap.size() > MAX_BLOCK_CACHE) {
                ...
                currentMap.put(NukkitLevel.localBlockHash(x, y, z), changeBlocksPresent);
            }
        }
    }
    ...
    public void setBlockIdAt(int x, int y, int z, int id) {
        this.blockCache.remove(NukkitLevel.blockHash(x, y, z));
        ...
        if (this.chunkPopulationQueue.containsKey(index)) {
            ...
            for (int xx = -1; xx <= 1; ++xx) {
                for (int zz = -1; zz <= 1; ++zz) {
                    this.chunkPopulationLock.remove(NukkitLevel.chunkHash(x + xx, z + zz));
                }
            }
    ...
    public boolean isChunkLoaded(int x, int z) {
        return this.chunks.containsKey(NukkitLevel.chunkHash(x, z)) || this.provider.isChunkLoaded(x, z);
    }
    ...
    public void cancelUnloadChunkRequest(int x, int z) {
        this.unloadQueue.remove(NukkitLevel.chunkHash(x, z));
    }
    ...
    public boolean populateChunk(int x, int z, boolean force) {
        ...
        if (!chunk.isPopulated()) {
            ...
            for (int xx = -1; xx <= 1; ++xx) {
                for (int zz = -1; zz <= 1; ++zz) {
                    if (this.chunkPopulationLock.containsKey(NukkitLevel.chunkHash(x + xx, z + zz))) {
            ...
            if (populate) {
                if (!this.chunkPopulationQueue.containsKey(index)) {
                    ...
                    for (int xx = -1; xx <= 1; ++xx) {
                        for (int zz = -1; zz <= 1; ++zz) {
                            this.chunkPopulationLock.put(NukkitLevel.chunkHash(x + xx, z + zz), true);
                        }
                    }
    ...
    public void doChunkGarbageCollection() {
        ...
        for (FullChunk chunk : new ArrayList<>(this.provider.getLoadedChunks().values())) {
            if (!this.chunks.containsKey(NukkitLevel.chunkHash(chunk.getX(), chunk.getZ()))) {
    ...
    public boolean setRaining(boolean raining) {
        ...
        NukkitServer.broadcastPacket(this.getPlayers().values(), pk);
