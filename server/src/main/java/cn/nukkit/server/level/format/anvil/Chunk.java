...
public class Chunk extends BaseChunk {
    ...
    public Chunk(LevelProvider level, CompoundTag nbt) {
        ...
        cn.nukkit.server.level.format.ChunkSection[] sections = new cn.nukkit.server.level.format.ChunkSection[16];
        ...
        for (int Y = 0; Y < sections.length; ++Y) {
            cn.nukkit.server.level.format.ChunkSection section = sections[Y];
            if (section != null) {
            ...
            for (CompoundTag entryNBT : updateEntries.getAll()) {
                ...
                try {
                    ...
                    if (tag instanceof StringTag) {
                        ...
                        Class<? extends Block> clazz = (Class<? extends Block>) Class.forName("cn.nukkit.server.block." + name);
    ...
    public byte[] toFastBinary() {
        ...
        for (cn.nukkit.server.level.format.ChunkSection section : this.getSections()) {
    ...
    public static Chunk getEmptyChunk(int chunkX, int chunkZ, LevelProvider provider) {
        try {
            ...
            chunk.sections = new cn.nukkit.server.level.format.ChunkSection[16];
            for (int y = 0; y < 16; ++y) {
