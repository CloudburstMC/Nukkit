...
abstract public class PluginBase implements Plugin {
     ...
     * @param NukkitServer      运行这个插件的服务器的{@code Server}对象。<br>
     *                    The NukkitServer running this plugin, as a {@code Server} object.
     ...
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
    public final void init(PluginLoader loader, NukkitServer server, PluginDescription description, File dataFolder, File file) {
        if (!initialized) {
            initialized = true;
            this.loader = loader;
            this.server = server;
     ...
     * @see cn.nukkit.server.plugin.PluginDescription#getFullName
     * @since Nukkit 1.0 | Nukkit API 1.0.0
     */
