...
public class PluginManager {
    ...
    public PluginManager(NukkitServer server, SimpleCommandMap commandMap) {
        this.server = server;
    ...
    public boolean registerInterface(Class<? extends PluginLoader> loaderClass) {
        if (loaderClass != null) {
            try {
                Constructor constructor = loaderClass.getDeclaredConstructor(NukkitServer.class);
                ...
                if (pattern.matcher(file.getName()).matches()) {
                    ...
                    if (description != null) {
                        try {
                            ...
                            NukkitServer.getInstance().getLogger().debug("Could not load plugin", e);
                            return null;
                        }
                    }
                }
