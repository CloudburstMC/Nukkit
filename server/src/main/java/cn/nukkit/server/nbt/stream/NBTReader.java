...
import static cn.nukkit.server.nbt.NBTEncodingType.MCPE;
...
public class NBTReader implements Closeable {
    ...
    public NBTReader(DataInput input, NBTEncodingType encoding) {
    ...
    private Tag<?> deserialize(TagType type, boolean skipName, int depth) throws IOException {
        ...
        if (type != TagType.END && !skipName) {
            int length = encoding == MCPE ? input.readByte() & 0xFF : input.readShort();
        ...
        switch (type) {
            ...
            case INT:
                int in = encoding == MCPE ? VarInt.readSignedInt(input) : input.readInt();
            ...
            case BYTE_ARRAY:
                int arraySz1 = encoding == MCPE ? VarInt.readSignedInt(input) : input.readInt();
                byte[] valueBytesBa = new byte[arraySz1];
                input.readFully(valueBytesBa);
                return new ByteArrayTag(tagName, valueBytesBa);
            case STRING:
                int length = encoding == MCPE ? input.readByte() : input.readUnsignedShort();
            ...
            case LIST:
                ...
                int listLength = encoding == MCPE ? VarInt.readSignedInt(input) : input.readInt();
            ...
            case INT_ARRAY:
                int arraySz2 = encoding == MCPE ? VarInt.readSignedInt(input) : input.readInt();
