...
import cn.nukkit.server.raknet.protocol.packet.PING_DataPacket;
import cn.nukkit.server.raknet.server.RakNetServer;
...
public class RakNetInterface implements ServerInstance, AdvancedSourceInterface {
    ...
    public RakNetInterface(NukkitServer server) {
        this.server = server;

        this.raknet = new RakNetServer(log, this.server.getPort(), this.server.getIp().equals("") ? "0.0.0.0" : this.server.getIp());
    ...
    public void setName(String name) {
        ...
        this.handler.sendOption("name",
                "MCPE;" + Utils.rtrim(names[0].replace(";", "\\;"), '\\') + ";" +
                        ...
                        NukkitServer.getGamemodeString(this.server.getDefaultGamemode(), true) + ";");
    }
            ...
            if (packet.pid() == ProtocolInfo.BATCH_PACKET) {
                ...
                try {
                    buffer = Zlib.deflate(
                            ...
                            NukkitServer.getInstance().networkCompressionLevel);
                } catch (Exception e) {
