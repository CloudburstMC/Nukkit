...
import cn.nukkit.server.network.util.NativeCodeFactory;
...
public class QueryDirectPacketHandler extends SimpleChannelInboundHandler<DirectAddressedQueryPacket> {
    ...
    public QueryDirectPacketHandler(NukkitServer server) {
        this.server = server;
    }
    ...
    public void refreshInfo() {
        ...
        event.getPlugins().forEach(pluginContainer -> pluginJoiner.add(pluginContainer.getName() + " " + pluginContainer.getVersion()));
        ...
        ImmutableMap<String, String> kvs = ImmutableMap.<String, String>builder()
                ...
                .put("plugins", "Nukkit " + server.getVersion() + "-API-" + server.getApiVersion() +
