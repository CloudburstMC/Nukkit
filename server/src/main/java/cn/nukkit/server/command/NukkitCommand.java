...
import cn.nukkit.server.command.data.CommandData;
 ...
 * Nukkit Project
 */
public abstract class NukkitCommand implements Command{
    ...
    public static void broadcastCommandMessage(CommandSender source, String message, boolean sendToSource) {
        Set<Permissible> users = source.getServer().getPluginManager().getPermissionSubscriptions(NukkitServer.BROADCAST_CHANNEL_ADMINISTRATIVE);

        TranslatedMessage result = new TranslatedMessage("chat.type.admin", new String[]{source.getName(), message});

        TranslatedMessage colored = new TranslatedMessage(TextFormat.GRAY + "" + TextFormat.ITALIC + "%chat.type.admin", new String[]{source.getName(), message});
    ...
    public static void broadcastCommandMessage(CommandSender source, Message message) {
        broadcastCommandMessage(source, message, true);
    }

    public static void broadcastCommandMessage(CommandSender source, Message message, boolean sendToSource) {
    ...
    public static final CommandData generateDefaultData() {
        if (defaultDataTemplate == null) {
            //defaultDataTemplate = new Gson().fromJson(new InputStreamReader(NukkitServer.class.getClassLoader().getResourceAsStream("command_default.json")));
        }
    ...
    public abstract boolean execute(CommandExecutorSource sender, String commandLabel, String[] args);

    public boolean testPermission(CommandExecutorSource target) {
        if (this.testPermissionSilent(target)) {
            return true;
        }

        if (!(target instanceof MessageRecipient)) {
            return false;
        }

        if (this.permissionMessage == null) {
            ((MessageRecipient) target).sendMessage(new TranslatedMessage(TextFormat.RED + "%commands.generic.unknown", this.name));
        } else if (!this.permissionMessage.equals("")) {
            ((MessageRecipient) target).sendMessage(this.permissionMessage.replace("<permission>", this.permission));
        }

        return false;
    }

    public boolean testPermissionSilent(CommandExecutorSource target) {
