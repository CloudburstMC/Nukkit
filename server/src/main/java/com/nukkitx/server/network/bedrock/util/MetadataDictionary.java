package com.nukkitx.server.network.bedrock.util;
...
public class MetadataDictionary {
    ...
    public void writeTo(ByteBuf buf) {
        VarInts.writeUnsignedInt(buf, typeMap.size());
    ...
    public static MetadataDictionary deserialize(ByteBuf buf) {
        ...
        int count = VarInts.readUnsignedInt(buf);
        for (int i = 0; i < count; i++) {
            int idx = VarInts.readUnsignedInt(buf);
            MetadataConstants.Type type = MetadataConstants.Type.values()[VarInts.readUnsignedInt(buf)];
    ...
    private static void serialize(ByteBuf buf, int idx, Object o) {
        ...
        VarInts.writeUnsignedInt(buf, idx);
        VarInts.writeUnsignedInt(buf, type.ordinal());
        type.write(buf, o);
    }
