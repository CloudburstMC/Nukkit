...
import com.nukkitx.server.network.raknet.codec.DatagramRakNetDatagramCodec;
import com.nukkitx.server.network.raknet.codec.DatagramRakNetPacketCodec;
import com.nukkitx.server.network.raknet.handler.ExceptionHandler;
import com.nukkitx.server.network.raknet.handler.RakNetDatagramHandler;
import com.nukkitx.server.network.raknet.handler.RakNetPacketHandler;
import com.nukkitx.server.util.NativeUtil;
...
public class MinecraftNetworkListener extends ChannelInitializer<DatagramChannel> implements NetworkListener {
    ...
    public boolean bind() {
        ...
        int threadCount;
        if (NativeUtil.isReusePortAvailable()) {
            threadCount = netThreads < 1 ? Runtime.getRuntime().availableProcessors() : netThreads;
        } else {
            threadCount = 1;
        }
