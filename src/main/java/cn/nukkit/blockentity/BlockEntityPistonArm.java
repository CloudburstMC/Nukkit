...
public class BlockEntityPistonArm extends BlockEntitySpawnable {
    ...
    private void moveCollidedEntities() {
        ...
        for (BlockVector3 pos : this.attachedBlocks) {
            BlockEntity blockEntity = this.level.getBlockEntity(new Vector3(pos.getSide(pushDir).getX(), pos.getSide(pushDir).getY(), pos.getSide(pushDir).getZ()));
    ...
    public boolean onUpdate() {
        ...
        if (this.progress == this.lastProgress) {
            ...
            for (BlockVector3 pos : this.attachedBlocks) {
                BlockEntity movingBlock = this.level.getBlockEntity(new Vector3(pos.getSide(pushDir).getX(), pos.getSide(pushDir).getY(), pos.getSide(pushDir).getZ()));
