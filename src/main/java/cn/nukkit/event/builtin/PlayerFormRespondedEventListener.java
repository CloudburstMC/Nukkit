            ...
            if ((response != null) && (response.getResponses().size() > 0)) {
                ...
                if (entity instanceof BlockEntityCommand) {
                    ...
                    if (response.getDropdownResponse(1) != null) {
                        ...
                        log.info("Setting type to {}", type);
                        commandEntity.setType(type);

                        Vector3 pos = commandEntity.getLocation().asBlockVector3().asVector3();
                        // Vector3 dir =
                        // commandEntity.getBlock().getLocation().getDirectionVector().normalize();

                        log.info("x: {}, y: {}, z: {}", pos.x, pos.y, pos.z);
                        // log.info("dx: {}, dy: {}, dz: {}",dir.x, dir.y,
                        // dir.z);
                    ...
                    if (response.getDropdownResponse(2) != null) {
                        ...
                        log.info("Setting redstone to {}", redstone);
                    ...
                    if (response.getDropdownResponse(3) != null) {
                        ...
                        log.info("After - Block Damage: {} vs. New Direction: {}", block.getDamage(), newDirection);
                    }

                    log.info("Command Block Entity set command:{}, type:{}, conditional: {}, redstone:{} by player {}", commandEntity.getCommand(), commandEntity.getType(), commandEntity.getConditional(), commandEntity.getRedstone(), event.getPlayer().getName());
                    event.getPlayer().getLevel().save();

                    if (commandEntity.getType() == 2) {
                        log.info("type here: {}", commandEntity.getType());
