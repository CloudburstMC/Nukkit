 ...
 * 监听类中监听事件的方法称作<b>监听方法</b>。一个监听类可以包含多个不同的监听方法。
 ...
 * Methods with specified parameters should be written in order to listen events. This method is called a <b>handler method</b>.
 * One listener class could contain many different handler methods.
 * After implemented the listener class, plugin should register it in plugin manager.</p>
 *
 * <p>事件监听器被注册后，Nukkit在对应的事件发生时，会使用反射来调用监听类中对应的方法。反射识别的是监听类中监听</p>
 *
 * <p>这是一个编写监听类和监听方法的例子。注意的是，标签{@code @EventHandler}和参数的类型是必需的：<br>
 * Here is an example for writing a listener class and a handler method.
 * Note that for the handler method, tag {@code @EventHandler} and the parameter is required:</p>
 * <pre>
