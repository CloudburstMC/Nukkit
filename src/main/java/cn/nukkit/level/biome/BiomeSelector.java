...
public class BiomeSelector {
    ...
    public BiomeSelector(final NukkitRandom random) {
    ...
    public Biome pickBiome(final int x, final int z) {
        ...
        final float noiseOcean = this.ocean.noise2D(x, z, true);
        final float noiseRiver = this.river.noise2D(x, z, true);
        final float temperature = this.temperature.noise2D(x, z, true);
        final float rainfall = this.rainfall.noise2D(x, z, true);
        final EnumBiome biome;
        if (noiseOcean < -0.15f) {
            if (noiseOcean < -0.91f) {
                if (noiseOcean < -0.92f) {
            ...
            final float hills = this.hills.noise2D(x, z, true);
