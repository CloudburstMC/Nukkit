            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (isTall) {
                    for (int y = startY; y < maxY; y++) {
                        //center column
                        level.setBlockIdAt(x, y, z, PACKED_ICE);
                        //t shape
                        level.setBlockIdAt(x + 1, y, z, PACKED_ICE);
                        level.setBlockIdAt(x - 1, y, z, PACKED_ICE);
                        level.setBlockIdAt(x, y, z + 1, PACKED_ICE);
                        level.setBlockIdAt(x, y, z - 1, PACKED_ICE);
                        //additional blocks on the side
                        if (random.nextBoolean()) {
                            level.setBlockIdAt(x + 1, y, z + 1, PACKED_ICE);
                        }
                        if (random.nextBoolean()) {
                            level.setBlockIdAt(x + 1, y, z - 1, PACKED_ICE);
                        }
                    ...
                    //finish with a point
                    level.setBlockIdAt(x + 1, maxY, z, PACKED_ICE);
                    level.setBlockIdAt(x - 1, maxY, z, PACKED_ICE);
                    level.setBlockIdAt(x, maxY, z + 1, PACKED_ICE);
                    level.setBlockIdAt(x, maxY, z - 1, PACKED_ICE);
                        ...
                        for (int xx = (int) -currWidth; xx < currWidth; xx++) {
                            for (int zz = (int) -currWidth; zz < currWidth; zz++) {
                                ...
                                if ((int) currWidth != currDist && random.nextBoolean()) {
                                    level.setBlockIdAt(x + xx, y, z + zz, PACKED_ICE);
                                }
                            }
                        }
