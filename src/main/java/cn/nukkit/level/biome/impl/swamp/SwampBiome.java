...
public class SwampBiome extends GrassyBiome {
    ...
    public SwampBiome() {
        ...
        final PopulatorLilyPad lilypad = new PopulatorLilyPad();
        lilypad.setBaseAmount(4);
        lilypad.setRandomAmount(2);
        this.addPopulator(lilypad);

        final SwampTreePopulator trees = new SwampTreePopulator();
        trees.setBaseAmount(2);
        this.addPopulator(trees);

        final PopulatorFlower flower = new PopulatorFlower();
        flower.setBaseAmount(2);
        flower.addType(BlockID.RED_FLOWER, BlockFlower.TYPE_BLUE_ORCHID);
        this.addPopulator(flower);

        final MushroomPopulator mushroom = new MushroomPopulator(1);
        mushroom.setBaseAmount(-5);
        mushroom.setRandomAmount(7);
        this.addPopulator(mushroom);

        final PopulatorSmallMushroom smallMushroom = new PopulatorSmallMushroom();
