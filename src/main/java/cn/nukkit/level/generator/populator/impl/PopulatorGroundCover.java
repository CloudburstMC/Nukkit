...
public class PopulatorGroundCover extends Populator {
    ...
    public void populate(final ChunkManager level, final int chunkX, final int chunkZ, final NukkitRandom random, final FullChunk chunk) {
        final int realX = chunkX << 4;
        final int realZ = chunkZ << 4;
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                final Biome realBiome = EnumBiome.getBiome(chunk.getBiomeId(x, z));
                if (realBiome instanceof CoveredBiome) {
                    ...
                    synchronized (biome.synchronizeCover) {
                        ...
                        final int coverBlock = biome.getCoverBlock() << 4;

                        boolean hasCovered = false;
                        int realY;
                        //start one below build limit in case of cover blocks
                        for (int y = 254; y > 32; y--) {
                            if (chunk.getFullBlock(x, y, z) == PopulatorGroundCover.STONE) {
                                COVER:
                                if (!hasCovered) {
                                    if (y >= Normal.seaHeight) {
                                        chunk.setFullBlockId(x, y + 1, z, coverBlock);
                                        final int surfaceDepth = biome.getSurfaceDepth(y);
                                        for (int i = 0; i < surfaceDepth; i++) {
                                            realY = y - i;
                                            if (chunk.getFullBlock(x, realY, z) == PopulatorGroundCover.STONE) {
                                    ...
                                    final int groundDepth = biome.getGroundDepth(y);
