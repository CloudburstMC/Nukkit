...
public class PopulatorRavines extends Populator {
    ...
    public void populate(final ChunkManager level, final int chunkX, final int chunkZ, final NukkitRandom random, final FullChunk chunk) {
        ...
        final int i = this.checkAreaSize;

        for (int x = chunkX - i; x <= chunkX + i; x++) {
            for (int z = chunkZ - i; z <= chunkZ + i; z++) {
                final long l3 = x * this.worldLong1;
                final long l4 = z * this.worldLong2;
    ...
    protected void generateChunk(final int chunkX, final int chunkZ, final FullChunk generatingChunkBuffer) {
        ...
        final double d1 = chunkX * 16 + this.random.nextInt(16);
        final double d2 = PopulatorRavines.numberInRange(this.random, this.ravineMinAltitude, this.ravineMaxAltitude);
        final double d3 = chunkZ * 16 + this.random.nextInt(16);
        ...
        for (int j = 0; j < i; j++) {
            ...
            final float f3 = (this.random.nextFloat() * 2.0F + this.random.nextFloat()) * 2.0F;

            final int size = PopulatorRavines.numberInRange(this.random, this.ravineMinLength, this.ravineMaxLength);

            this.createRavine(this.random.nextLong(), generatingChunkBuffer, d1, d2, d3, f3, f1, f2, size, this.ravineDepth);
        }
    }

    protected void createRavine(final long paramLong, final FullChunk generatingChunkBuffer, double paramDouble1, double paramDouble2, double paramDouble3,
                                final float paramFloat1, float paramFloat2, float paramFloat3, final int size, final double paramDouble4) {
        final Random localRandom = new Random(paramLong);

        final int chunkX = generatingChunkBuffer.getX();
        final int chunkZ = generatingChunkBuffer.getZ();

        final double d1 = chunkX * 16 + 8;
        final double d2 = chunkZ * 16 + 8;
        ...
        for (int stepCount = 0; stepCount < size; stepCount++) {
            ...
            final float f4 = MathHelper.cos(paramFloat3);
            final float f5 = MathHelper.sin(paramFloat3);
            ...
            final double d5 = paramDouble1 - d1;
            final double d6 = paramDouble3 - d2;
            final double d7 = size - stepCount;
            final double d8 = paramFloat1 + 2.0F + 16.0F;
                ...
                for (int localZ = i2; i4 == 0 && localZ < i3; localZ++) {
                    for (int localY = minY + 1; i4 == 0 && localY >= maxY - 1; localY--) {
                        ...
                        if (localY < this.worldHeightCap) {
                            final int materialAtPosition = generatingChunkBuffer.getBlockId(localX, localY, localZ);
                            if (materialAtPosition == BlockID.WATER
            ...
            for (int localX = k; localX < m; localX++) {
                final double d9 = (localX + chunkX * 16 + 0.5D - paramDouble1) / d3;
                for (int localZ = i2; localZ < i3; localZ++) {
                    final double d10 = (localZ + chunkZ * 16 + 0.5D - paramDouble3) / d3;
                    if (d9 * d9 + d10 * d10 < 1.0D) {
                        for (int localY = minY; localY >= maxY; localY--) {
                            final double d11 = ((localY - 1) + 0.5D - paramDouble2) / d4;
                            if ((d9 * d9 + d10 * d10) * this.a[localY - 1] + d11 * d11 / 6.0D < 1.0D) {
                                final int material = generatingChunkBuffer.getBlockId(localX, localY, localZ);
                                if (material == BlockID.GRASS) {
                                    if (localY - 1 < 10) {
                                        generatingChunkBuffer.setBlock(localX, localY, localZ, BlockID.LAVA);
                                    } else {
