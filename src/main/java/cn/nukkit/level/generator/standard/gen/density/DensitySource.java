...
public interface DensitySource extends GenerationPass {
     ...
     * @return the density at the given coordinates
     */
    default double[] get(double[] arr, @NonNull BiomeMap biomes, int x, int y, int z, int sizeX, int sizeY, int sizeZ, int stepX, int stepY, int stepZ) {
        int totalSize = PValidation.ensurePositive(sizeX) * PValidation.ensurePositive(sizeY) * PValidation.ensurePositive(sizeZ);
