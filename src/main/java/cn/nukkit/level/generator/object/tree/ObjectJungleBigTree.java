...
public class ObjectJungleBigTree extends HugeTreesGenerator {
        ...
        if (!this.ensureGrowable(level, rand, position, height)) {
            ...
            for (int j = position.getY() + height - 2 - rand.nextInt(4); j > position.getY() + height / 2; j -= 2 + rand.nextInt(4)) {
                ...
                for (int k1 = j - j2; k1 <= j; ++k1) {
                    int l1 = k1 - j;
                    this.growLeavesLayer(level, Vector3i.from(k, k1, l), 1 - l1);
                }
            }
            ...
            for (int i2 = 0; i2 < height; ++i2) {
                ...
                if (this.canGrowInto(level.getBlockId(blockpos))) {
                    level.setBlockAt(blockpos, this.woodMetadata);
                ...
                if (i2 < height - 1) {
                    ...
                    if (this.canGrowInto(level.getBlockId(blockpos1))) {
                        level.setBlockAt(blockpos1, this.woodMetadata);
                    ...
                    if (this.canGrowInto(level.getBlockId(blockpos2))) {
                        level.setBlockAt(blockpos2, this.woodMetadata);
                    ...
                    if (this.canGrowInto(level.getBlockId(blockpos3))) {
                        level.setBlockAt(blockpos3, this.woodMetadata);
    ...
    private void placeVine(ChunkManager level, BedrockRandom random, Vector3i pos, int meta) {
        if (random.nextInt(3) > 0 && level.getBlockId(pos) == AIR) {
            level.setBlockAt(pos, Block.get(BlockIds.VINE, meta));
        }
    }
