...
public class Normal extends Generator {
    ...
    static {
        for (int i = -2; i <= 2; ++i) {
            for (int j = -2; j <= 2; ++j) {
                Normal.biomeWeights[i + 2 + (j + 2) * 5] = (float) (10.0F / Math.sqrt((float) (i * i + j * j) + 0.2F));
            }
        }
    }
    ...
    public Normal(final Map<String, Object> options) {
    ...
    public void init(final ChunkManager level, final NukkitRandom random) {
        ...
        final PopulatorGroundCover cover = new PopulatorGroundCover();
        this.generationPopulators.add(cover);

        final PopulatorBedrock bedrock = new PopulatorBedrock();
        ...
        final PopulatorCaves caves = new PopulatorCaves();
    ...
    public void generateChunk(final int chunkX, final int chunkZ) {
        ...
        final float[] depthRegion = this.depthNoise.generateNoiseOctaves(this.depthRegion.get(), chunkX * 4, chunkZ * 4, 5, 5, 200f, 200f, 0.5f);
        this.depthRegion.set(depthRegion);
        final float[] mainNoiseRegion = this.mainPerlinNoise.generateNoiseOctaves(this.mainNoiseRegion.get(), chunkX * 4, 0, chunkZ * 4, 5, 33, 5, 684.412f / 60f, 684.412f / 160f, 684.412f / 60f);
        this.mainNoiseRegion.set(mainNoiseRegion);
        final float[] minLimitRegion = this.minLimitPerlinNoise.generateNoiseOctaves(this.minLimitRegion.get(), chunkX * 4, 0, chunkZ * 4, 5, 33, 5, 684.412f, 684.412f, 684.412f);
        this.minLimitRegion.set(minLimitRegion);
        final float[] maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseOctaves(this.maxLimitRegion.get(), chunkX * 4, 0, chunkZ * 4, 5, 33, 5, 684.412f, 684.412f, 684.412f);
        this.maxLimitRegion.set(maxLimitRegion);
        final float[] heightMap = this.heightMap.get();
        ...
        for (int xSeg = 0; xSeg < 5; ++xSeg) {
            for (int zSeg = 0; zSeg < 5; ++zSeg) {
                ...
                final Biome biome = this.pickBiome(baseX + xSeg * 4, baseZ + zSeg * 4);

                for (int xSmooth = -2; xSmooth <= 2; ++xSmooth) {
                    for (int zSmooth = -2; zSmooth <= 2; ++zSmooth) {
                        final Biome biome1 = this.pickBiome(baseX + xSeg * 4 + xSmooth, baseZ + zSeg * 4 + zSmooth);
                        final float baseHeight = biome1.getBaseHeight();
                        final float heightVariation = biome1.getHeightVariation();

                        float scaledWeight = Normal.biomeWeights[xSmooth + 2 + (zSmooth + 2) * 5] / (baseHeight + 2.0F);
                ...
                final float heightVariationClone = heightVariationSum;
                baseHeightClone = baseHeightClone + depthNoise * 0.2f;
                baseHeightClone = baseHeightClone * 8.5f / 8.0f;
                final float baseHeightFactor = 8.5f + baseHeightClone * 4.0f;
                ...
                for (int ySeg = 0; ySeg < 33; ++ySeg) {
                    ...
                    final float minScaled = minLimitRegion[horizCounter] / 512f;
                    final float maxScaled = maxLimitRegion[horizCounter] / 512f;
                    final float noiseScaled = (mainNoiseRegion[horizCounter] / 10.0f + 1.0f) / 2.0f;
                    float clamp = MathHelper.denormalizeClamp(minScaled, maxScaled, noiseScaled) - baseScale;

                    if (ySeg > 29) {
                        final float yScaled = (float) (ySeg - 29) / 3.0F;
        ...
        for (int xSeg = 0; xSeg < 4; ++xSeg) {
            final int xScale = xSeg * 5;
            final int xScaleEnd = (xSeg + 1) * 5;

            for (int zSeg = 0; zSeg < 4; ++zSeg) {
                final int zScale1 = (xScale + zSeg) * 33;
                final int zScaleEnd1 = (xScale + zSeg + 1) * 33;
                final int zScale2 = (xScaleEnd + zSeg) * 33;
                final int zScaleEnd2 = (xScaleEnd + zSeg + 1) * 33;
                ...
                for (int ySeg = 0; ySeg < 32; ++ySeg) {
                    ...
                    final double height5 = (heightMap[zScale1 + ySeg + 1] - height1) * 0.125f;
                    final double height6 = (heightMap[zScaleEnd1 + ySeg + 1] - height2) * 0.125f;
                    final double height7 = (heightMap[zScale2 + ySeg + 1] - height3) * 0.125f;
                    final double height8 = (heightMap[zScaleEnd2 + ySeg + 1] - height4) * 0.125f;

                    for (int yIn = 0; yIn < 8; ++yIn) {
                        double baseIncr = height1;
                        double baseIncr2 = height2;
                        final double scaleY = (height3 - height1) * 0.25f;
                        final double scaleY2 = (height4 - height2) * 0.25f;

                        for (int zIn = 0; zIn < 4; ++zIn) {
                            final double scaleZ = (baseIncr2 - baseIncr) * 0.25f;
                            ...
                            for (int xIn = 0; xIn < 4; ++xIn) {
                                ...
                                } else if (ySeg * 8 + yIn <= Normal.seaHeight) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                final Biome biome = this.selector.pickBiome(baseX | x, baseZ | z);
    ...
    public Biome pickBiome(final int x, final int z) {
