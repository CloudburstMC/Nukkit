...
import cn.nukkit.world.particle.HugeExplodeSeedParticle;
...
public class Explosion {
    ...
    public boolean explodeB() {
        ...
        for (Block block : this.affectedBlocks) {
            ...
            } else if ((container = block.getWorld().getBlockEntity(block)) instanceof InventoryHolder) {
            ...
            for (BlockFace side : BlockFace.values()) {
                ...
                if (!this.affectedBlocks.contains(sideBlock) && !updateBlocks.contains(index)) {
                    ...
                    this.level.getServer().getPluginManager().callEvent(ev);
                    if (!ev.isCancelled()) {
                        ev.getBlock().onUpdate(World.BLOCK_UPDATE_NORMAL);
                    }
        ...
        this.level.addLevelSoundEvent(source, WorldSoundEventPacket.SOUND_EXPLODE);
