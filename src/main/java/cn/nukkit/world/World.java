...
import cn.nukkit.world.format.Chunk;
import cn.nukkit.world.format.ChunkSection;
...
import cn.nukkit.world.format.generic.BaseLevelProvider;
import cn.nukkit.world.format.generic.EmptyChunkSection;
...
import cn.nukkit.world.generator.PopChunkManager;
import cn.nukkit.world.generator.task.GenerationTask;
import cn.nukkit.world.generator.task.LightPopulationTask;
import cn.nukkit.world.generator.task.PopulationTask;
import cn.nukkit.world.particle.DestroyBlockParticle;
...
//    private final List<BlockUpdateEntry> nextTickUpdates = Lists.newArrayList();
        ...
        public Generator init() {
            try {
                ...
                if (Server.getInstance().isPrimaryThread()) {
                    generator.init(World.this, rand);
                }
    ...
    public World(Server server, String name, String path, Class<? extends WorldProvider> provider) {
        ...
        try {
            if (convert) {
                ...
                this.provider = provider.getConstructor(World.class, String.class).newInstance(this, newPath);
            } else {
                this.provider = provider.getConstructor(World.class, String.class).newInstance(this, path);
            }
        } catch (Exception e) {
            throw new WorldException("Caused by " + Utils.getExceptionMessage(e));
        }
        ...
        if (convert) {
            ...
            WorldProvider old = this.provider;
            try {
                this.provider = new WorldProviderConverter(this, path)
                        .from(old)
                        .to(Anvil.class)
                        .perform();
            } catch (IOException e) {
        ...
        this.provider.updateWorldName(name);
    ...
    public void close() {
        ...
        this.server.getWorlds().remove(this.levelId);
     ...
     * @param type ID of the sound from {@link cn.nukkit.network.protocol.WorldSoundEventPacket}
    ...
    public void addLevelSoundEvent(Vector3 pos, int type, int data, String identifier, boolean isBaby, boolean isGlobal) {
        WorldSoundEventPacket pk = new WorldSoundEventPacket();
        pk.sound = type;
        pk.extraData = data;
    ...
    public boolean unload(boolean force) {
        ...
        World defaultLevel = this.server.getDefaultWorld();
    ...
    public Map<Integer, Player> getChunkPlayers(int chunkX, int chunkZ) {
        long index = World.chunkHash(chunkX, chunkZ);
    ...
    private void performThunder(long index, FullChunk chunk) {
        ...
        if (ThreadLocalRandom.current().nextInt(10000) == 0) {
            ...
            this.addLevelSoundEvent(vector, WorldSoundEventPacket.SOUND_THUNDER, -1, EntityLightning.NETWORK_ID);
            this.addLevelSoundEvent(vector, WorldSoundEventPacket.SOUND_EXPLODE, -1, EntityLightning.NETWORK_ID);
        }
    }
    ...
    public void sendBlockExtraData(int x, int y, int z, int id, int data, Player[] players) {
        WorldEventPacket pk = new WorldEventPacket();
        pk.evid = WorldEventPacket.EVENT_SET_DATA;
        pk.x = x + 0.5f;
        pk.y = y + 0.5f;
        pk.z = z + 0.5f;
        pk.data = (data << 8) | id;
    ...
    public void sendBlocks(Player[] target, Vector3[] blocks, int flags, int dataLayer, boolean optimizeRebuilds) {
        ...
        for (Vector3 b : blocks) {
            ...
            if (optimizeRebuilds) {
                long index = World.chunkHash((int) b.x >> 4, (int) b.z >> 4);
        ...
        if (!this.loaders.isEmpty()) {
            for (ChunkLoader loader : this.loaders.values()) {
                ...
                for (int chunk = 0; chunk < chunksPerLoader; ++chunk) {
                    ...
                    long hash = World.chunkHash(dx + chunkX, dz + chunkZ);
                    ...
                    if (this.useSections) {
                        ...
                        for (int Y = 0; Y < 8 && (Y < 3 || blockTest != 0); ++Y) {
                            ...
                            for (int i = 0; i < tickSpeed; ++i) {
                                ...
                                int blockId = fullId >> 4;
                                blockTest |= fullId;
                                if (World.randomTickBlocks[blockId]) {
    ...
    public boolean save(boolean force) {
        ...
        this.server.getPluginManager().callEvent(new WorldSaveEvent(this));
    ...
    public void addLightUpdate(int x, int y, int z) {
        ...
        currentMap.put(World.localBlockHash(x, y, z), changeBlocksPresent);
    }
...
//        }
        ...
        int cx = x >> 4;
        int cz = z >> 4;
        long index = World.chunkHash(cx, cz);
        if (direct) {
    ...
    private void addBlockChange(int x, int y, int z) {
        long index = World.chunkHash(x >> 4, z >> 4);
        addBlockChange(index, x, y, z);
    }
    ...
    public Item useItemOn(Vector3 vector, Item item, BlockFace face, float fx, float fy, float fz, Player player, boolean playSound) {
        ...
        if (player != null) {
            ...
            if (!ev.isCancelled()) {
                ...
                if(item.getId() == ItemID.BUCKET && ItemBucket.getDamageByTarget(item.getDamage()) == BlockID.WATER) {
                    player.getWorld().sendBlocks(new Player[]{player}, new Block[]{Block.get(Block.AIR, 0, target)}, UpdateBlockPacket.FLAG_ALL_PRIORITY, 1);
                }
                return null;
            }
        ...
        if (playSound) {
            this.addLevelSoundEvent(hand, WorldSoundEventPacket.SOUND_PLACE, GlobalBlockPalette.getOrCreateRuntimeId(hand.getId(), hand.getDamage()));
        }
        ...
        if (this.chunkPopulationQueue.containsKey(index)) {
            ...
            for (int xx = -1; xx <= 1; ++xx) {
                for (int zz = -1; zz <= 1; ++zz) {
                    this.chunkPopulationLock.remove(World.chunkHash(x + xx, z + zz));
                }
            }
    ...
    public void removeEntity(Entity entity) {
        if (entity.getWorld() != this) {
            throw new WorldException("Invalid Entity level");
        }
    ...
    public void addBlockEntity(BlockEntity blockEntity) {
        if (blockEntity.getWorld() != this) {
            throw new WorldException("Invalid Block Entity level");
        }
    ...
    public boolean isChunkInUse(int x, int z) {
        return isChunkInUse(World.chunkHash(x, z));
    }
    ...
    public void cancelUnloadChunkRequest(int x, int z) {
        this.cancelUnloadChunkRequest(World.chunkHash(x, z));
    }
    ...
    public boolean populateChunk(int x, int z, boolean force) {
        ...
        if (!chunk.isPopulated()) {
            ...
            for (int xx = -1; xx <= 1; ++xx) {
                for (int zz = -1; zz <= 1; ++zz) {
                    if (this.chunkPopulationLock.containsKey(World.chunkHash(x + xx, z + zz))) {
            ...
            if (populate) {
                if (!this.chunkPopulationQueue.containsKey(index)) {
                    ...
                    for (int xx = -1; xx <= 1; ++xx) {
                        for (int zz = -1; zz <= 1; ++zz) {
                            this.chunkPopulationLock.put(World.chunkHash(x + xx, z + zz), Boolean.TRUE);
                        }
                    }
    ...
    public boolean setRaining(boolean raining) {
        ...
        if (raining) {
            pk.evid = WorldEventPacket.EVENT_START_RAIN;
            int time = ThreadLocalRandom.current().nextInt(12000) + 12000;
            pk.data = time;
            setRainTime(time);
        } else {
            pk.evid = WorldEventPacket.EVENT_STOP_RAIN;
            setRainTime(ThreadLocalRandom.current().nextInt(168000) + 12000);
        }
    ...
    public boolean setThundering(boolean thundering) {
        ...
        // These numbers are from Minecraft
        if (thundering) {
            pk.evid = WorldEventPacket.EVENT_START_THUNDER;
            int time = ThreadLocalRandom.current().nextInt(12000) + 3600;
            pk.data = time;
            setThunderTime(time);
        } else {
            pk.evid = WorldEventPacket.EVENT_STOP_THUNDER;
            setThunderTime(ThreadLocalRandom.current().nextInt(168000) + 12000);
        }
...
//        }
    ...
    public boolean createPortal(Block target) {
        ...
        if (sizeX >= 2 && sizeX <= maxPortalSize) {
            ...
            this.addLevelSoundEvent(target, WorldSoundEventPacket.SOUND_IGNITE);
            return true;
