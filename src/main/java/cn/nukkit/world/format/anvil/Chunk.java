package cn.nukkit.world.format.anvil;
...
import cn.nukkit.world.format.anvil.palette.BiomePalette;
import cn.nukkit.world.format.generic.BaseChunk;
...
public class Chunk extends BaseChunk {
    ...
    public Chunk(WorldProvider level) {
        this(level, null);
    }

    public Chunk(Class<? extends WorldProvider> providerClass) {
        this((WorldProvider) null, null);
        this.providerClass = providerClass;
    }

    public Chunk(Class<? extends WorldProvider> providerClass, CompoundTag nbt) {
        this((WorldProvider) null, nbt);
        this.providerClass = providerClass;
    }

    public Chunk(WorldProvider level, CompoundTag nbt) {
        this.provider = level;
        if (level != null) {
        ...
        if (nbt == null) {
            ...
            this.sections = new cn.nukkit.world.format.ChunkSection[16];
        ...
        if (updateEntries != null && updateEntries.size() > 0) {
            for (CompoundTag entryNBT : updateEntries.getAll()) {
                ...
                this.provider.getWorld().scheduleUpdate(block, block, entryNBT.getInt("t"), entryNBT.getInt("p"), false);
            }
        }
    ...
    public static Chunk fromBinary(byte[] data, WorldProvider provider) {
        try {
    ...
    public static Chunk fromFastBinary(byte[] data, WorldProvider provider) {
        try {
    ...
    public byte[] toFastBinary() {
        ...
        for (cn.nukkit.world.format.ChunkSection section : this.getSections()) {
        ...
        Set<BlockUpdateEntry> entries = this.provider.getWorld().getPendingBlockUpdates(this);

        if (entries != null) {
            ListTag<CompoundTag> tileTickTag = new ListTag<>("TileTicks");
            long totalTime = this.provider.getWorld().getCurrentTick();
    ...
    public int getBlockSkyLight(int x, int y, int z) {
        cn.nukkit.world.format.ChunkSection section = this.sections[y >> 4];
        if (section instanceof cn.nukkit.world.format.anvil.ChunkSection) {
            cn.nukkit.world.format.anvil.ChunkSection anvilSection = (cn.nukkit.world.format.anvil.ChunkSection) section;
    ...
    public static Chunk getEmptyChunk(int chunkX, int chunkZ, WorldProvider provider) {
        try {
            Chunk chunk;
            if (provider != null) {
...
//            chunk.lightPopulated = false;
    ...
    public boolean compress() {
        ...
        for (cn.nukkit.world.format.ChunkSection section : getSections()) {
